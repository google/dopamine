...
"""Tests for dopamine.tf.agents.rainbow.rainbow_agent."""
...
class RainbowAgentTest(tf.test.TestCase):
  ...
  def testShapesAndValues(self):
    with tf.compat.v1.Session() as sess:
      ...
      self.assertEqual(
          agent._net_outputs.probabilities.shape,
          agent._net_outputs.logits.shape,
      )
      self.assertEqual(
      ...
      self.assertEqual(
          agent._replay_net_outputs.logits.shape[2], self._num_atoms
      )
      self.assertEqual(
          agent._replay_next_target_net_outputs.logits.shape[1],
          self._num_actions,
      )
      self.assertEqual(
      ...
      self.assertEqual(
          agent._net_outputs.q_values.shape, (1, self._num_actions)
      )
