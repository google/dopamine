...
def create_runner(base_dir, schedule='continuous_train_and_eval'):
  ...
  elif schedule == 'jax_train':
    return JaxRunner(base_dir, create_agent)
  else:
...
class JaxRunner(Runner):
  """Object that handles running training-only experiments with Jax.
  """
  ...
  def __init__(  # pylint: disable=super-init-not-called
      ...
      evaluation_steps=125000,
      max_steps_per_episode=27000):
    """Initialize the JaxRunner object in charge of running a full experiment.
    ...
    - Reload from the latest checkpoint, if available, and initialize the
      Checkpointer object.
    """
    logging.info('Creating JaxRunner ...')
    assert base_dir is not None
    self._logging_file_prefix = logging_file_prefix
    self._log_every_n = log_every_n
    self._num_iterations = num_iterations
    ...
    self._base_dir = base_dir
    self._create_directories()
    self._summary_writer = tensorboard.SummaryWriter(self._base_dir)
    self._environment = create_environment_fn()
    self._agent = create_agent_fn(None, self._environment,
                                  summary_writer=self._summary_writer)

    self._initialize_checkpointer_and_maybe_resume(checkpoint_file_prefix)
    # We turn off evaluation by default.
    self._agent.eval_mode = False
  ...
  def _save_tensorboard_summaries(self, iteration, num_episodes,
                                  average_reward):
    """Save statistics as tensorboard summaries."""
    self._summary_writer.scalar('Train/NumEpisodes', num_episodes, iteration)
    self._summary_writer.scalar(
        'Train/AverageReturns', average_reward, iteration)
