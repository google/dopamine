...
def train(
  ...
  bellman_target = base_dqn_agent.target_q(
  ...
  actual_bellman_target = base_dqn_agent.target_q(
      q_online_temp, next_states, rewards, terminals, cumulative_gamma
  )
  _, (actual_bellman_loss, _, _, _, _) = loss_fn(
      online_params, actual_bellman_target
  )
      ...
      if (
        ...
        if self.summary_writer is not None:
          with self.summary_writer.as_default():
            tf.summary.scalar(
                'Losses/Aggregate', loss, step=self.training_steps
            )
            tf.summary.scalar(
                'Losses/Bellman', bellman_loss, step=self.training_steps
            )
            tf.summary.scalar('Losses/DR3', dr3_loss, step=self.training_steps)
            tf.summary.scalar(
                'Losses/CQLLoss', cql_loss, step=self.training_steps
            )
            tf.summary.scalar(
                'Losses/BellmanActual',
                actual_bellman_loss,
                step=self.training_steps,
            )
            tf.summary.scalar(
                'Losses/q_vals_data', q_data, step=self.training_steps
            )
            tf.summary.scalar(
                'Losses/q_vals_pi', q_pi, step=self.training_steps
            )
