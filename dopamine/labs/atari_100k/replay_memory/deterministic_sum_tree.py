...
def parallel_stratified_sample(rng, nodes, i, n, depth):
  """Sample a batch of indices in parallel with vmap.

  Args:
    rng: a JAX prng key
    nodes: the sum tree's storage array
    i: the ID of the current search (index in batch)
    n: batch size
    depth: the depth of the sum tree

  Returns:
    indices: a vector of indices, of shape (n,).
  """
